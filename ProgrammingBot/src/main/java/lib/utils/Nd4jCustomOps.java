...
public class Nd4jCustomOps {
    public static INDArray arange(double start, double end, double step) {
        int elements = (int) ((end - start) / step);
        System.out.println(elements);
        return Nd4j.linspace(start, start + elements * step,  elements + 1, DataType.FLOAT);
    }

    public static INDArray randInt(int [] shape, int upper) {
        return Transforms.floor(Nd4j.rand(shape).mul(upper));
    }

    public static String type(INDArray arr) {
        return arr.data().dataType().name();
    }

    public static String arrayInfo(INDArray arr) {
        return arr.data().dataType().toString();
    }

    public static INDArray resize(INDArray arr, int [] shape) {
        INDArray resized = Nd4j.create(shape);
        resized.get(NDArrayIndex.createCoveringShape(arr.shape())).assign(arr);
        return resized;
    }

    public static boolean Equal(INDArray arr1, INDArray arr2) {
    ...
    public static INDArray [] hsplit(INDArray arr1, int numOfSplits) {
        return split(arr1, numOfSplits, 1);
    }

    public static INDArray compare(INDArray arr1, INDArray arr2, Predicate<Boolean []> predicate) {
